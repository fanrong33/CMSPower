...
class Page extends Think {
    ...
    public function __construct($totalRows,$listRows,$parameter='') {
        ...
        $this->nowPage  = !empty($_GET[C('VAR_PAGE')])?abs($_GET[C('VAR_PAGE')]):1;
    ...
    public function show() {
        ...
        // 到第  页 确定
        $goto = "";
        $goto .= "<form action='' method='get'>";
        foreach($params as $k => $v){
        	if($v == '') continue;
        	$goto .= "<input type='hidden' name='$k' value='$v'>";
        }
        $goto .= "到第 <input type='text' class='ipt-text' name='".$p."' value='".$this->nowPage."' style='width:25px;'> 页 <button class='btn btn-small' type='submit'>确定</button>";
		$goto .= "</form>";
        
        $pageStr	 =	 str_replace(
            array('%header%','%nowPage%','%totalRow%','%totalPage%','%upPage%','%downPage%','%first%','%prePage%','%linkPage%','%nextPage%','%end%','%goto%'),
            array($this->config['header'],$this->nowPage,$this->totalRows,$this->totalPages,$upPage,$downPage,$theFirst,$prePage,$linkPage,$nextPage,$theEnd, $goto),
            $this->config['theme']);
        return $pageStr;
    }
